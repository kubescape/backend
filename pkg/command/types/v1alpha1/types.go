...
type OperatorCommandSpec struct {
	...
	// labels should include - command type, node name, app name (node-agent, operator,kubescape, ..)
	Designators []identifiers.PortalDesignator `json:"designators,omitempty"` // Designators are the designators for the command (on which the command will be executed)
	// TODO: Body - (suggestion to use gob) https://pkg.go.dev/encoding/gob
	// TODO: Add TTL
	Args         map[string]interface{} `json:"args,omitempty"`         // Args are the arguments for the command
	CommandIndex *int                   `json:"commandIndex,omitempty"` // CommandIndex is the index of the command in the sequence of commandd that are part of the same command group
	CommandCount *int                   `json:"commandCount,omitempty"` // CommandCount is the total number of commands in the sequence of commands that are part of the same command group
}
...
type OperatorCommandStatus struct {
	...
	Executer    string                      `json:"executer,omitempty"`    // Executer is the name of the entity that executed the command
